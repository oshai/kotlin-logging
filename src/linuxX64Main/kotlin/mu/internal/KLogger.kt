...
internal class NativeLogger(
    ...
    override fun trace(msg: () -> Any?) = Warn.isLoggingEnabled(msg(), config) { config.appender.warn(config, it) }

    override fun debug(msg: () -> Any?) = Debug.isLoggingEnabled(msg(), config) { config.appender.debug(config, it) }

    override fun info(msg: () -> Any?) = Info.isLoggingEnabled(msg(), config) { config.appender.info(config, it) }

    override fun warn(msg: () -> Any?) = Warn.isLoggingEnabled(msg(), config) { config.appender.warn(config, it) }

    override fun error(msg: () -> Any?) = Error.isLoggingEnabled(msg(), config) { config.appender.error(config, it) }

    // With Throwable
    override fun trace(t: Throwable?, msg: () -> Any?) =
        Warn.isLoggingEnabled(msg(), config) { config.appender.warn(config, it) }

    override fun debug(t: Throwable?, msg: () -> Any?) =
        Debug.isLoggingEnabled(msg(), config) { config.appender.debug(config, it) }

    override fun info(t: Throwable?, msg: () -> Any?) =
        Info.isLoggingEnabled(msg(), config) { config.appender.info(config, it) }

    override fun warn(t: Throwable?, msg: () -> Any?) =
        Warn.isLoggingEnabled(msg(), config) { config.appender.warn(config, it) }

    override fun error(t: Throwable?, msg: () -> Any?) =
        Error.isLoggingEnabled(msg(), config) { config.appender.error(config, it) }
    // End Throwable
