...
internal object KLoggerFactory {
  ...
  internal inline fun logger(loggable: KLoggable): mu.two.KLogger =
      logger(KLoggerNameResolver.name(loggable.javaClass))

  /** get logger by explicit name */
  internal inline fun logger(name: String): mu.two.KLogger = wrapJLogger(jLogger(name))

  /** get logger for the method, assuming it was declared at the logger file/class */
  internal inline fun logger(noinline func: () -> Unit): mu.two.KLogger =
  ...
  internal inline fun wrapJLogger(jLogger: Logger): mu.two.KLogger =
